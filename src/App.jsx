...
import { checkARSupport } from './utils/ar';
...
return;
        ...
        <aside className="viewer-side">
          <div className="panel">
            <div className="panel-head">
              <div className="panel-title">Select Fruit</div>
              <input
            ...
            <div className="selector-grid">
              {filteredFruits.map((fruit) => (
                <button
                  ...
                  className={`selector-card ${selectedFruit?.name === fruit.name ? 'active' : ''}`}
                  onClick={() => {
                    setSelectedFruit(fruit);
                    setDetectionStatus(''); 
                  }}
                >
                  <span className="fruit-avatar small">{fruit.emoji || '🍎'}</span>
                  <div className="selector-info">
                    <div className="selector-name">{fruit.name}</div>
                    <div className="selector-sub">3D ready</div>
                  </div>
                </button>
              ))}
            </div>
  ...
  <div className="detect-status">
    {detectionStatus
      .replace(/^Detected from image:\s*(.+)$/i, 'Detected "$1" from image')
      .replace(/^Detected from filename:\s*(.+)$/i, 'Detected "$1" from image')
    }
  </div>
)}
