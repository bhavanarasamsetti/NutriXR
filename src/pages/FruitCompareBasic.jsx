    ...
    async function loadFruits() {
      try {
       ...
       /* const list = mod.default || []; 
        ...
        setCompareFruits(defaults); original */

        // 🔴 DEMO MODE: restrict fruits shown in Compare
// const list = mod.default || []; // ← original (keep for later)

const list = (mod.default || []).filter(f =>
  DEMO_FRUITS.includes(f.name)
);

setFruits(list);

// Default first two demo fruits for comparison
const defaults = [list[0]?.name, list[1]?.name].filter(Boolean);
    ...
    <div className="compare-page" ref={pageRef}>
      <header className="compare-nav">
        <div className="nav-left">
          <button className="nav-back" onClick={() => navigate("/app")}> 
            ...
            <div className="modal-grid">
              {fruits.map((fruit) => {
                ...
                return (
                  ...
                  >
                    <div className="modal-fruit-icon">{fruit.emoji || "dY?Z"}</div>
